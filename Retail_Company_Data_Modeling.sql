...
DROP DATABASE IF EXISTS retail_company_db;		# Dropping a database
-- DROP DATABASE retail_company_db;				# Dropping a database
CREATE DATABASE retail_company_db;
-- OR
-- DROP SCHEMA IF EXISTS retail_company_db;		# Dropping a schema
-- DROP SCHEMA retail_company_db;				# Dropping a schema
-- CREATE SCHEMA retail_company_db;
-- Note: In MySQL world, Schema and Databse is same. But in other softwares/environments, these two might be different from each other.

USE retail_company_db;							# pointing to or using the database/schema created
...
-- sales_fact: approach 1
CREATE TABLE sales_fact (
-- 	attribute		datatype	constraints
-- 	-----------------------------------------
    order_id		INT			PRIMARY KEY,	# order_id attribute is declared as primary key
    customer_id		INT			NOT NULL,
    order_date		DATE,
    store_id		SMALLINT
);

DROP TABLE IF EXISTS sales_fact;				# Dropping a table

-- sales_fact: approach 2
CREATE TABLE sales_fact (
-- 	attribute		datatype	constraints
-- 	-----------------------------------------
    order_id		INT,						# order_id attribute is declared without recognizing it as primary key
    customer_id		INT			NOT NULL,
    order_date		DATE,
    store_id		SMALLINT,
...
-- 	-----------------------------------------
    PRIMARY KEY (order_id)						# order_id attribute is separately declared as primary key
);

-- order_product_mapping: approach 1
CREATE TABLE order_product_mapping (
	order_id			INT,
    product_id			VARCHAR (10),
    quantity_ordered	INT,
    
    PRIMARY KEY (order_id, product_id)			# order_id and product_id are declared as composite primary key during the table creation
);

DROP TABLE IF EXISTS order_product_mapping;		# Dropping a table

-- order_product_mapping: approach 2
CREATE TABLE order_product_mapping (
	order_id 			INT,
	product_id 			VARCHAR (10),
	quantity_ordered 	INT
...
ADD PRIMARY KEY (order_id, product_id);			# order_id and product_id are declared composite primary key after the table creation
...
-- 	-------------------------------------------------
    ...
    list_price			DECIMAL (12,2),					# DECIMAL (noOfTotalDigits, noOfDigitsAfterDecimalPoint)
    sale_price			DECIMAL (12,2)					# DECIMAL (noOfTotalDigits, noOfDigitsAfterDecimalPoint)
);
...
****************************/
-- sales_fact - order_product_mapping:
ALTER TABLE sales_fact
-- 				constraint name
ADD CONSTRAINT	order_id_fk
-- 			primary key				table name with foreign key
...
-- sales_fact: approach 3
-- alternate approach to create foreign keys for sales_fact table towards other tables:
CREATE TABLE sales_fact (
-- 	attribute		datatype	constraints
-- 	-----------------------------------------
    order_id		INT			PRIMARY KEY,	# order_id attribute is declared as primary key
...
-- 	CONSTRAINT	(attributes)	Table reference with foreign key attribute
-- 	-----------------------------------------
	FOREIGN KEY (order_id)		REFERENCES order_product_mapping (order_id)		ON DELETE CASCADE ON UPDATE CASCADE,
    FOREIGN KEY (customer_id)	REFERENCES customer_info (customer_id)			ON DELETE CASCADE ON UPDATE CASCADE,
    FOREIGN KEY (store_id)		REFERENCES branch_details (store_id)			ON DELETE CASCADE ON UPDATE CASCADE
);
-- Note: Only difference in approach 3 for sales_fact table is that the names of FKs are seen as a little different in schema view.
...
-- 1. How many transactions have been done by each gender?

SELECT COUNT(DISTINCT order_id) FROM sales_fact;
# 550 unique order records

SELECT COUNT(DISTINCT customer_id) FROM sales_fact;
# 101 unique customer records

-- no of orders associated with male customers
SELECT COUNT(s.order_id) AS transactions
FROM sales_fact s
JOIN customer_info c
ON s.customer_id = c.customer_id
WHERE c.gender = 'male';
# Conclusion: 358 orders from male customers

-- no of orders associated with female customers
SELECT COUNT(s.order_id) AS transactions
FROM sales_fact s
JOIN customer_info c
ON s.customer_id = c.customer_id
WHERE c.gender = 'female';
# Conclusion: 192 orders from female customers

# Note: 358 male + 192 female becomes 550 total customers.


-- 2. How many customers have placed an order from more than one store?

-- customer ids associated with more than 1 store ids
SELECT customer_id, COUNT(store_id) AS no_of_stores
FROM sales_fact
GROUP BY customer_id
HAVING no_of_stores > 1
ORDER BY no_of_stores DESC;

-- Number of customer ids associated with more than 1 store ids
...
# Conclusion: All the 101 customers have placed an order from more than one store.


-- 3. Which city has the highest number of orders?

-- Number of orders recorded in sales_fact table
SELECT COUNT(DISTINCT order_id) FROM sales_fact;
# 550 unique orders

-- Number of cities recorded in branch_details
SELECT COUNT(DISTINCT city) FROM branch_details;
# 3 unique cities

-- Number of orders associated with each city
SELECT b.city, COUNT(s.order_id) AS order_count
...
# Conclusion: Bengaluru has the highest number of orders.


-- 4. Which area has the highest sales?

-- Number of areas recorded in branch_details
SELECT COUNT(DISTINCT area) FROM branch_details;
# 8 unique areas

-- Area with highest sale
SELECT b.area, SUM(p.sale_price) AS sale_amount
FROM branch_details b
INNER JOIN sales_fact s ON b.store_id = s.store_id
INNER JOIN order_product_mapping opm ON s.order_id = opm.order_id
INNER JOIN product_info p ON p.product_id = opm.product_id
GROUP BY b.area
...
-- Number of customer ids recorded in sales_fact
SELECT COUNT(DISTINCT customer_id) FROM sales_fact;
# 101 unique customer ids

-- Number of state recorded in branch_details
SELECT COUNT(DISTINCT state) FROM branch_details;
# 3 unique states

-- 3 unique states are:
SELECT DISTINCT state FROM branch_details;
# 3 states are Karnataka, Maharashtra and Haryana

-- customer ids linked with 1 or more states
SELECT s.customer_id, COUNT(DISTINCT b.state) AS no_of_states
FROM sales_fact s
INNER JOIN branch_details b
ON s.store_id = b.store_id
GROUP BY s.customer_id
ORDER BY no_of_states DESC;
# many customers are associated with more than 1 state

-- state with highest customers
SELECT b.state, COUNT(DISTINCT s.customer_id) AS unique_customer_count
FROM branch_details b
INNER JOIN sales_fact s ON b.store_id = s.store_id
GROUP BY b.state
